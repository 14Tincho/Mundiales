...
if ($penales1 == NULL && $penales2 == NULL) {
    $sql = "UPDATE cuartos SET goles = $goles1 WHERE nropartido = $nropartido AND posicion = 1";
    $rs = mysqli_query($link, $sql);
    $sql = "UPDATE cuartos SET goles = $goles2 WHERE nropartido = $nropartido AND posicion = 2";
    $rs = mysqli_query($link, $sql);
}else {    
    $sql = "UPDATE cuartos SET goles = $goles1, penales = $penales1 WHERE nropartido = $nropartido AND posicion = 1";
    $rs = mysqli_query($link, $sql);
    $sql = "UPDATE cuartos SET goles = $goles2, penales = $penales2 WHERE nropartido = $nropartido AND posicion = 2";
    $rs = mysqli_query($link, $sql);
}
mysqli_close($link);
